...
# Public API
    @classmethod
    def addSection(cls, section, title):
    ...
    @classmethod
    def removeSection(cls, section):
        if section not in cls.section_names and section not in cls.prefs:
    ...
    def addPage(self, section, widget):
        """Adds a `widget` to the internal stack.

        Be sure to call addSection method before using this function.
    ...
    def __add_plugin_manager_section(self):
        ...
        page.show_all()
        self.addSection("plugins", _("Plugins"))
        self.addPage("plugins", page)
    ...
    def __add_shortcuts_section(self):
        ...
        self.addSection("shortcuts", _("Shortcuts"))
        self.addPage("shortcuts", outside_box)
...
class PluginPreferencesRow(Gtk.Bin):
    def __init__(self, model, group, title_group_size, switch_group_size,
                 show_builtins=True):
        Gtk.Bin.__init__(self)
        self.app = model.app
        self.preferences_group = group
    ...
    def _pack_widgets(self):
        self._pack_title_widgets()
        self._container.pack_end(self.switch, False, False, 0)

    def _pack_title_widgets(self):
    ...
    def _connect_signals(self):
        self.switch.connect("notify::active", self.__switch_active_cb)

    def _setup_switches(self):
        loaded_plugins = self.app.plugin_manager.engine.get_loaded_plugins()
        if self.plugin_info.get_module_name() in loaded_plugins:
            self.switch.set_active(True)

    def _setup_group_sizes(self, title_group_size, switch_group_size):
        title_group_size.add_widget(self._title_box)
        switch_group_size.add_widget(self.switch)

    def _design(self):
        self._design_margins()
        self._design_title_widgets()
        self._design_switch()

    def _design_margins(self):
    ...
    def _design_title_widgets(self):
    ...
    def _design_switch(self):
        # self._switch.props.xalign = 0
        self.switch.props.margin_left = PADDING

    def __switch_active_cb(self, switch, unused_pspec):
        engine = self.app.plugin_manager.engine
        if switch.get_active():
            if not engine.load_plugin(self.plugin_info):
                stack = self.app.gui.preferences_dialog.stack
                preferences_page = stack.get_child_by_name("plugins")
                msg = _("Unable to load the plugin '%s'" %
                        self.plugin_info.get_module_name())
                preferences_page.show_infobar(msg, Gtk.MessageType.WARNING)
                switch.set_active(False)
        else:
            dependant_plugins = self._get_dependant_plugins()
            for row in self.preferences_group.get_rows():
                if row.plugin_info in dependant_plugins:
                    row.switch.set_active(False)
            self.app.plugin_manager.engine.unload_plugin(self.plugin_info)
        if self.app.gui.preferences_dialog is None:
            return
        page = self.app.gui.preferences_dialog.stack.get_child_by_name("plugins")
        self.app.gui.preferences_dialog.stack.set_property("visible-child",
                                                           page)

    def _get_dependant_plugins(self):
        dependant_plugins = []
        module_name = self.plugin_info.get_module_name()
...
class PluginPreferencesGroup(Gtk.ListBox):
    def __init__(self, list_store, *args, **kwargs):
        Gtk.ListBox.__init__(self, *args, **kwargs)
        self._preferences_rows = []

        self.app = list_store.app
        self.list_store = list_store
        self.title_size_group = Gtk.SizeGroup(mode=Gtk.SizeGroupMode.HORIZONTAL)
        self.switch_size_group = Gtk.SizeGroup(mode=Gtk.SizeGroupMode.HORIZONTAL)
    ...
    def _create_widget_func(self, item, unused_user_data):
        preference_row = PluginPreferencesRow(item, self, self.title_size_group,
                                              self.switch_size_group)
        return preference_row

    def __plugin_loaded_cb(self, engine, plugin_info):
        dependencies = plugin_info.get_dependencies()
        for row in self.get_rows():
            if row.plugin_info.get_module_name() in dependencies:
                if not row.plugin_info.is_loaded():
...
class PluginPreferencesPage(Gtk.ScrolledWindow):
    ...
    def __init__(self, app):
        ...
        preferences_group = PluginPreferencesGroup(list_store)
        ...
        self._wrapper_box.pack_start(preferences_group, False, False, 0)
